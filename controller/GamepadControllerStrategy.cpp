...
void GamepadControllerStrategy::handleInput(float deltaTime) {
    ...
    if(isAboveDeadzone(vector3d0, threshold)) {
        ...
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_0_X, vector3d0.x, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_0_Y, vector3d0.y, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_0_Z, vector3d0.z, deltaTime));
    }

    if(isAboveDeadzone(vector3d1, threshold)) {
        eventManager.publish<Axis3dEvent>(Axis3dEvent(EVENT_VECTOR_3D_1, vector3d1, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_1_X, vector3d1.x, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_1_Y, vector3d1.y, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_1_Z, vector3d1.z, deltaTime));
    }

    if(isAboveDeadzone(vector3d2, threshold)) {
        eventManager.publish<Axis3dEvent>(Axis3dEvent(EVENT_VECTOR_3D_2, vector3d2, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_2_X, vector3d2.x, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_2_Y, vector3d2.y, deltaTime));
        eventManager.publish<FloatEvent>(FloatEvent(EVENT_FLOAT_2_Z, vector3d2.z, deltaTime));
    }
